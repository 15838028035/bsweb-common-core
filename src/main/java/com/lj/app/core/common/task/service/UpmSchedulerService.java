      ...
      public void run() {
        ...
        if (baseTaskService != null) {
          try {
            ...
            e.getMessage();
            log.error("upmSchedulerService run " + baseTaskService.getClass().getName() + "error, error msg: "
                + e.getMessage());
          }
        }

        log.warn("第" + (i) + "次开始执行操作... " + "时间：【" + new SimpleDateFormat("yyyy-MM-dd hh:mm:ss.SSS").format(new Date())
            + "】");
      }
    }, new Trigger() {
      @Override
