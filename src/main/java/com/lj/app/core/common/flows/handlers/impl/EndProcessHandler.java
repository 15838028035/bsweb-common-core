...
public class EndProcessHandler implements IHandler {
	...
	public void handle(Execution execution) {
		try {
			...
			List<FlowTask> tasks = engine.flowQueryService().getActiveTasks(
					order.getId());
			for (FlowTask task : tasks) {
				if (task.isMajor())
					throw new FlowException("存在未完成的主办任务,请确认.");
				engine.FlowTaskServiceApi().complete(task.getId().toString(),
						FlowEngine.AUTO);
			}
			engine.flowOrderService().complete(order.getId().toString());

			if (StringUtil.isNotBlank(order.getParentId())) {
				FlowOrder parentOrder = engine.flowQueryService().getFlowOrder(
						order.getParentId());
				if (parentOrder == null)
					return;
				FlowProcess process = engine.flowProcessService()
						.getProcessById(parentOrder.getFlowProcessId());
				ProcessModel pm = process.getModel();
				if (pm == null)
					return;
				SubProcessModel spm = (SubProcessModel) pm.getNode(order
						.getParentNodeName());
				Execution newExecution = new Execution(engine, process,
						parentOrder, execution.getArgs());
