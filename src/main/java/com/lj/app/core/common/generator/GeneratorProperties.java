...
public class GeneratorProperties {
	...
	public static void reload() {
		try {
			GLogger.info("Start Load GeneratorPropeties from classpath:"
					+ Arrays.toString(PROPERTIES_FILE_NAMES));
			Properties p = new Properties();
			String[] loadedFiles = PropertiesHelper
					.loadAllPropertiesFromClassLoader(p, PROPERTIES_FILE_NAMES);
			GLogger.info("GeneratorPropeties Load Success,files:"
					+ Arrays.toString(loadedFiles));
			...
			throw new RuntimeException("Load " + PROPERTIES_FILE_NAMES
					+ " error", e);
		}
	}
		...
		if (loadedFiles != null && loadedFiles.length > 0) {
			...
			if (basedir != null && basedir.startsWith(".")) {
				p.setProperty("basedir",
						new File(new File(loadedFiles[0]).getParent(), basedir)
								.getAbsolutePath());
			}
		}
	}
	...
	public static boolean getBoolean(GeneratorConstants key) {
		return getHelper().getBoolean(key.code,
	...
	public static void setProperties(Properties inputProps) {
		...
		for (Iterator it = propertiesHelper.entrySet().iterator(); it.hasNext();) {
			...
			GLogger.debug("[Property] " + entry.getKey() + "="
					+ entry.getValue());
		}
		GLogger.info("");
	}
