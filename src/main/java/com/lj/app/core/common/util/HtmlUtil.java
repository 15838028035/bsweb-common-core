...
public class HtmlUtil {
	...
	public static String list2Select(String id, List<String> list) {
		StringBuffer sb = new StringBuffer("<select id=\"").append(
				id == null ? "" : id).append("\">");

		for (Object s : list) {
			sb.append("<option value=\"").append(s).append("\">").append(s)
					.append("</option>");
		}
	...
	public static String getButton(String type, String value, String attribute) {
		...
		sb.append("<input type=\"button\" value=\"")
				.append(value == null ? "button" : value).append("\"")
	...
	public static String mapList2Select(String selectId, List<Map<String,Object>> list) {
		StringBuffer sb = new StringBuffer("<select id=\"").append(
				selectId == null ? "" : selectId).append("\">");

		for (Map<String,Object> map : list) {
			sb.append("<option value=\"").append(map.get("value"))
					.append("\">").append(map.get("label")).append("</option>");
	...
	public static String htmlStringToRealChar(String toChangeStr) {
		for (int i = 0; i < htmlString.length; i++) {
			toChangeStr = toChangeStr.replace(htmlString[i],
					String.valueOf(realChar[i]));
		}
		return toChangeStr;
	}
	...
	public static String realCharToHtmlString(String toChangeStr) {
		for (int i = 0; i < realChar.length; i++) {
			toChangeStr = toChangeStr.replace(String.valueOf(realChar[i]),
					htmlString[i]);
		}
		return toChangeStr;
	}
