...
public final class EncryptDSPropertiesFactory {
	...
	public synchronized static Properties getProperties(Boolean isPwdEncryped,
			String username, String password) throws Exception {
		Properties p = new Properties();
		p.setProperty(USER, username);

		/**
		 * 如果密码是加密的，那么进行解密。如果还没有解密，那么久进行解密后设置设置originalPassword=解密后的密码；
		 * 否则如果密码没有加密，就设置originalPassword=传进来的密码
		 */
		if (isPwdEncryped) {
