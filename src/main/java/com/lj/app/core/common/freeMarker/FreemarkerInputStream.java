...
public class FreemarkerInputStream extends InputStream {
	...
	public FreemarkerInputStream(InputStream input, Configuration conf, String encoding,
	...
	private ByteArrayInputStream processByFreemarker(InputStream input) {
		try {
			...
			InputStreamReader reader = StringUtils.isEmpty(encoding) ? new InputStreamReader(input)
			...
			return new ByteArrayInputStream(StringUtils.isEmpty(encoding) ? cachedTemplateOutput.toString().getBytes()
