...
public class UpmOrganizationServiceImpl<UpmOrganization> extends BaseServiceImpl<UpmOrganization> implements UpmOrganizationService<UpmOrganization>{
	...
	public List<UpmOrganization> findOrgByParentId(Integer treeNodeId) {
		/*UpmOrganization o = new UpmOrganization();
		UpmOrganization parent = new UpmOrganization();
		parent.setOrgId(treeNodeId);
		o.setParentOrg(parent);
		List<UpmOrganization> results = orgDao.findByExample(o,"sort_no,org_name");*/
		//return results;
		//FIXME
		return null;
	}

	public List<UpmOrganization> findOrgByOrgName(Integer treeNodeId,
			String orgName) {
		Map<String, String> map = new HashMap<String, String>();
		map.put("orgId", String.valueOf(treeNodeId));
		map.put("orgName", orgName);
		return queryForList(
				"getChilderOrgsByName", map);
	...
	public List<UpmOrganization> findUpmOrganizationByName(
			String organizationName, Long organizationId) {
		/*UpmOrganization uapOrg = new UpmOrganization();
		UpmOrganization uapOrgParent = new UpmOrganization();
		uapOrgParent.setOrgId(organizationId);
		uapOrg.setParentOrg(uapOrgParent);
		uapOrg.setOrgName(organizationName);
		return orgDao.findByExample2(uapOrg);*/
		//FIXME
		return null;
	}

	public List<UpmOrganization> findUapOrgByNameAndParentId(
			String organizationName, Long organizationId) {
	...
	public boolean hasChildOrg(Integer orgId) {
		/*UpmOrganization uapOrg = new UpmOrganization();
		UpmOrganization uapOrgParent = new UpmOrganization();
		uapOrgParent.setOrgId(orgId);
		uapOrg.setParentOrg(uapOrgParent);
		List<UpmOrganization> list = this.queryForList(uapOrg);
	...
	public String findSelfAndParentOrgTreeDataJson(Long mainAcctId)throws Exception {
		...
		/*List<UpmOrganization> orgs = orgDao.findSelfAndParentOrg(mainAcctId);
		if(null!=orgs && !orgs.isEmpty()){
			List<DefaultTreeNodeNoCheck> treeNodeList = new ArrayList<DefaultTreeNodeNoCheck>();
			for(int i=0; i<orgs.size(); i++){
				UpmOrganization up = orgs.get(i);
				String id = up.getOrgId()+"";
				String text = up.getOrgName();
				String parentId = up.getParentOrg()==null?null:up.getParentOrg().getOrgId()+"";
				treeNodeList.add(SimpleTreeNoCheck.createNew(id, text, parentId));
			}
			SimpleTreeNoCheck simpleTree = SimpleTreeNoCheck.valueOf(treeNodeList,"0");
			if(null!=simpleTree){
	 ...
	 * @return
	 */
	public List<UpmOrganization> findOrgListByName(Integer belongOrg,
			String orgName) {
		Map<String, String> map = new HashMap<String, String>();
	...
	@Override
	public UpmOrganization saveServiceOrg(UpmOrganization UpmOrganization,
			Long createMainacct, Long parentOrgid) {
		// TODO Auto-generated method stub
		return null;
	}
