...
public class JavaClass {
	...
	static String getClassName(String simpleJavaType) {
		return simpleJavaType.indexOf('$') >= 0 ? simpleJavaType
				.substring(simpleJavaType.indexOf('$') + 1) : simpleJavaType;
	}
	...
	public String getLastPackageNameFirstUpper() {
		return getLastPackageName() == null ? "" : StringHelper
				.capitalize(getLastPackageName());
	}
	...
	public String getSuperclassName() {
		return clazz.getSuperclass() != null ? clazz.getSuperclass().getName()
				: null;
	}
	...
	public JavaMethod[] getPublicNotStaticMethods() {
		Method[] staticMethods = filterByModifiers(clazz.getDeclaredMethods(),
				Modifier.STATIC);
		Method[] publicMethods = filterByModifiers(clazz.getDeclaredMethods(),
				Modifier.PUBLIC);
		Method[] filtered = exclude(publicMethods, staticMethods).toArray(
				new Method[0]);
	...
	public static class MavenHelper {
		public static String getMavenJavaTestSourceFile(String clazzFile) {
			...
			if (clazzFile.indexOf("target/classes") >= 0) {
				String result = StringHelper.replace(clazzFile,
						"target/classes", "src/test/java");
	...
	public JavaField getField(String name) throws NoSuchFieldException,
			SecurityException {
	...
	private Method[] filterByModifiers(Method[] methods,
