...
public class PropertyPlaceholderHelper {
		...
		while (startIndex != -1) {
			...
			if (endIndex != -1) {
				...
				if (propVal != null) {
					...
					GLogger.info("Resolved placeholder '" + placeholder + "'");
	...
	private int findPlaceholderEndIndex(CharSequence buf, int startIndex) {
		...
		while (index < buf.length()) {
			...
			} else if (StringHelper.substringMatch(buf, index,
	...
	public static class PropertyPlaceholderConfigurerResolver implements
			PlaceholderResolver {
			...
			if (value == null) {
				...
				if (value == null) {
					if (placeholderName.startsWith("env.")) {
						value = System.getenv(placeholderName.substring("env."
								.length()));
					}
				}
			}
