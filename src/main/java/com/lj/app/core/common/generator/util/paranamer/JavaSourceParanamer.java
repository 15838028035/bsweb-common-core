...
public class JavaSourceParanamer implements Paranamer {
	...
	public String[] lookupParameterNames(AccessibleObject methodOrConstructor,
		...
		try {
			...
			if (methodOrConstructor instanceof Method) {
				...
				javaSource = m.getDeclaringClass().getName().replace('.', '/')
						+ ".java";
			} else if (methodOrConstructor instanceof Constructor) {
				Constructor c = (Constructor) methodOrConstructor;
				javaSource = c.getDeclaringClass().getName().replace('.', '/')
						+ ".java";
			} else {
				throw new IllegalArgumentException("unknow AccessibleObject"
						+ methodOrConstructor
						+ ",must be Method or Constructor");
			}
			InputStream javaSourceInputStream = classLoader
					.getResourceAsStream(javaSource);
			try {
				if (javaSourceInputStream != null) {
					return parser.parseJavaFileForParamNames(
							methodOrConstructor,
			...
			if (throwExceptionIfMissing) {
				throw new RuntimeException(
						"IOException while reading javasource,method:"
	...
	public static class JavaSourceFileMethodParametersParser {
		public String[] parseJavaFileForParamNames(Constructor<?> constructor,
				String content) {
		...
		public String[] parseJavaFileForParamNames(Method method, String content) {
			return parseJavaFileForParamNames(content, method.getName(),
					method.getParameterTypes());
		}

		public String[] parseJavaFileForParamNames(
				AccessibleObject methodOrConstructor, String content) {
			if (methodOrConstructor instanceof Method) {
				return parseJavaFileForParamNames((Method) methodOrConstructor,
						content);
			} else if (methodOrConstructor instanceof Constructor) {
				return parseJavaFileForParamNames(
						(Constructor) methodOrConstructor, content);
			} else {
				throw new IllegalArgumentException("unknow AccessibleObject"
						+ methodOrConstructor
						+ ",must be Method or Constructor");
		...
		private String getParamsPattern(Class<?>[] parameterTypes) {
			...
			for (int i = 0; i < parameterTypes.length; i++) {
				...
				String paramPattern = ".*" + type.getSimpleName()
						+ ".*\\s+(\\w+).*";
