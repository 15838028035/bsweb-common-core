...
public class StartSubProcessHandler implements IHandler {
	...
	public void handle(Execution execution) {
		...
		try {
			...
			FlowProcess process = engine.flowProcessService()
					.getProcessByVersion(model.getProcessName(),
							model.getVersion());

			Execution child = execution.createSubExecution(execution, process,
					model.getName());
			...
			if (isFutureRunning) {
				...
				// 提交执行任务，并返回future
				Future<FlowOrder> future = es.submit(new ExecuteTask(execution,
						process, model.getName()));
				try {
					es.shutdown();
					order = future.get();
			...
			execution.addTasks(engine.flowQueryService().getActiveTasks(
					order.getId()));
		} catch (Exception e) {
	...
	class ExecuteTask implements Callable<FlowOrder> {
		 ...
		 */
		public ExecuteTask(Execution execution, FlowProcess process,
				String parentNodeName) {
			this.engine = execution.getEngine();
			child = execution.createSubExecution(execution, process,
					parentNodeName);
		}
