...
public class MethodParameter {
	...
	public MethodParameter(int paramIndex, JavaMethod method,
	...
	public static Paranamer paranamer = setParanamer(ClassHelper
	...
	public static String[] parseJavaFileForParamNames(Method method,
			File javaFile) throws IOException {
		String content = IOHelper.readFile(javaFile);
		return new JavaSourceFileMethodParametersParser()
				.parseJavaFileForParamNames(method, content);
	}
	...
	public static class JavaSourceFileMethodParametersParser {
		public String[] parseJavaFileForParamNames(Method method, String content) {
			Pattern methodPattern = Pattern.compile("(?s)" + method.getName()
					+ "\\s*\\(" + getParamsPattern(method) + "\\)\\s*\\{");
		...
		public static String getParamsPattern(Method method) {
			...
			for (int i = 0; i < method.getParameterTypes().length; i++) {
				...
				String classType = type.getSimpleName().replace("[", "\\[")
						.replace("]", "\\]");
