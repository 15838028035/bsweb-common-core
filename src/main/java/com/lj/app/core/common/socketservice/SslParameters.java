package com.lj.app.core.common.socketservice;
...
public class SslParameters {
	...
	protected SslParameters(String keyStoreFilename, String keyStorePassword,
	...
	protected void prepareGlobalConfiguration() {
		...
		setProperty("javax.net.ssl.keyStorePassword", keyStorePassword,
				"FitNesse42");
		setProperty("javax.net.ssl.trustStore", trustStoreFilename,
				"fitnesse.jks");

	}

	protected void restorePreviousConfiguration() {
		setProperty("javax.net.ssl.keyStore", keyStoreFilenameOld,
				keyStoreFilenameOld);
		setProperty("javax.net.ssl.keyStorePassword", keyStorePasswordOld,
				keyStorePasswordOld);
		setProperty("javax.net.ssl.trustStore", trustStoreFilenameOld,
				trustStoreFilenameOld);
	}
	...
	public static SslParameters setSslParameters(String sslParameterClassName) {
		...
		if (sslParameterClassName == null
				|| "true".equalsIgnoreCase(sslParameterClassName)) {
			sslParametersInstance = SslParameters.class;
		} else {
			try {
				sslParametersInstance = Class.forName(sslParameterClassName)
						.asSubclass(SslParameters.class);
			} catch (ClassNotFoundException e) {
				throw new RuntimeException(
						"Preparing SSL Parameters with Class "
								+ sslParameterClassName
								+ " failed. Class Not Found.", e);
			}
		}
		try {
			return sslParametersInstance.newInstance();
		} catch (Exception e) {
			throw new RuntimeException("Preparing SSL Parameters with Class "
					+ sslParameterClassName + " failed.", e);
		}
	}
